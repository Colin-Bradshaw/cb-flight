        ...
        stage('scan') { 
            steps {
                withSonarQubeEnv('SonarQubeScanner'){
                    sh 'mvn verify sonar:sonar -Dsonar.projectKey=cb-flight -Dsonar.host.url=http://jenkins.hitec.link:9000/ -Dsonar.login=3b4c1716c02efa3ef2d3c1e2d483ffbb714c024c'
                }
            }
        }
      stage('build') { 
